-- @block Get all LARs with more than one Owner change during asignment period
WITH lars_to_fix AS (
    SELECT
        id,
        account__c,
...
nb_owners AS (
    ...
    WHERE
        -- 72 LARs with duplicates (created same day)
...
-- @block Get the corresponding accounts (with LARs having more than one Owner during assignment period)
WITH lars_to_fix AS (
    SELECT
        id,
        account__c,
...
lars_with_multiple_assignment AS (
    SELECT 
        lar_id
    FROM nb_owners
    WHERE nb_assignments > 1)
...
JOIN lars_with_multiple_assignment l
    ON l.lar_id = bc.id

-- @block Find all LARs associated to accounts with LARs having more than one Owner during assignment period
WITH lars_to_fix AS (
    SELECT
        id,
        account__c,
...
SELECT
    ah.account__c,
    bc.id,
    bc.assignement_date__c,
    bc.end_relation_date__c,
    bc.owner_relation__c
FROM accounts_highlighted ah
JOIN staging_salesforce.batchaccountrelation__c bc
    ON ah.account__c = bc.account__c
ORDER BY 1,3